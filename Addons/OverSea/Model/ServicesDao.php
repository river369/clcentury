...
class ServicesDao extends BaseDao
{
   ...
   // To be change
    ...
    {
        $sql = 'SELECT * FROM yz_services WHERE service_area = :service_area AND (tag like "%' . $keyWord . '%" or seller_name like "%' . $keyWord . '%" or description like "%' . $keyWord . '%" or service_brief like "%'. $keyWord . '%" or service_area like "%'. $keyWord .'%") and status=60 order by id desc, stars desc limit ' . $pageIndexRange;
    ...
    {
        $sql = 'SELECT * FROM yz_services WHERE tag like "%' . $keyWord . '%" or seller_name like "%' . $keyWord . '%" or description like "%' . $keyWord . '%" or service_area like "%' . $keyWord .'%" or service_brief like "%' . $keyWord . '%" and status=60 order by id desc, stars desc limit ' . $pageIndexRange;
        Logs::writeClcLog(__CLASS__ . "," . __FUNCTION__ . ",sql=".$sql);
